      ...
      builder: (_) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Center(
                child: Image.network(product.imageUrl, width: 400, height: 150),
              ),
  ...
  showDialog(
    ...
    builder: (_) => FutureBuilder<List<dynamic>>(
      future: () {
        ...
        if (product != null) {
          futures.add(fetchProductsImage(product.productId!)); // productId phải khác null
        }
      ...
      if (snapshot.hasError) {
        ...
        final imageList = (product != null && lists.length > 3 && lists[3] != null)
        ? List<ProductImage>.from(lists[3])
        : <ProductImage>[];
        ...
        return Dialog(
          child: ProductForm(
            ...
            imageList: imageList,
            onSubmit: (prod,imageList) async {
              try {
                if (product == null) {
                  // Thêm sản phẩm mới
                await ProductService.createProduct(prod);
                List<Product> newProductList = await ProductService.fetchProducts();
                int maxProductId = 1;
                if (newProductList.isNotEmpty) {
                  int maxProductId = newProductList.map((p) => p.productId ?? 0).reduce(max);
                }
                await insertProductsImages(maxProductId, imageList);
                ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  await updateProductsImages(prod.productId!, imageList);
                ...
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Có lỗi khi lưu sản phẩm')),
                );
              }
            },
          ),
